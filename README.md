...
这一版把 IDE 的“学习环境”从单机原型推进到可以跟项目一起共享：

- 项目级代码便签：默认保存到项目根目录 `.ai-code-tutor/notes.json`，图片放在 `.ai-code-tutor/assets/`。这些文件可以直接提交 Git，其他人用 AI Code Tutor IDE 打开同一项目时能看到同一批便签。
- 旧便签迁移：如果本机旧版 `userData/code-notes.json` 中存在当前项目的便签，第一次读取时会按 ID 去重迁移到项目目录。
- 双位置便签继续保留：行号旁便签和代码内便签可以同时存在。
- 代码内便签只允许放在词法边界：不会再把 `getUser`、`priceMinor`、`user_name` 这种完整单词从中间切开；空格、标点、运算符、行首和行尾仍然可以放。
- 便签支持图片：可以点“＋ 图片”、粘贴截图或把图片拖进便签；缩略图可点击放大。
- 点击便签以外的位置会自动保存修改并收起；`Esc` 仍然作为放弃本次未保存修改的快速出口。
- 外观设置：多种背景颜色、自定义颜色、纯色 / 渐变背景、渐变起止色与方向、上传自己的背景图片、作用范围（仅编辑器 / 整个 IDE）、图片布局、位置、强度、暗色遮罩和模糊程度。选择“整个 IDE”时，标题栏、命令栏、文件树、标签栏和状态栏都会跟随背景色调；背景图片复制到 Electron `userData`，原图移动后仍然可用。
- Markdown：打开 `.md` / `.markdown` 文件时自动出现 `编辑 / 分栏 / 预览`，默认分栏并实时预览；支持标题、粗体、斜体、删除线、列表、任务列表、引用、表格、代码块、链接以及项目内相对路径图片。
- 命令区重新压缩到更紧凑的 52px 节奏，减少按钮下方无意义空白。
...
`.ai-code-tutor` 不会显示在 IDE 的普通项目文件树里，但它仍然是项目真实目录的一部分，可以正常被 Git 跟踪和分享。
