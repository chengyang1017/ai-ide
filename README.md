...
Alpha 0.13 没有新增 npm 包，所以从 Alpha 0.12 升级不需要重新 `npm install`。
...
1. 打开真实项目，把光标移到任意代码行，确认行号旁出现轻量 `＋`。
2. 点击 `＋`，输入一段自己的理解，按 `Ctrl+Enter` 保存；该行应变成 `📝`。
3. 再次点击 `📝`，确认原文字仍在，并测试编辑、保存和删除。
4. 关闭 IDE，再执行 `npm run dev`，恢复同一项目和文件后，确认 `📝` 和文字仍然存在。
5. 在便签所在行前面插入若干代码并保存，重启后确认便签会尽量按保存的代码锚点重新找到原位置。
6. 确认 `PROJECT / NAVIGATE / AI TUTOR` 标题和按钮之间已有明显间距。
7. 回归测试 Alpha 0.12：角色仍避开代码；Dart Ctrl+Click 仍有蓝色下划线反馈并跳到真实 Definition。
8. 修改真实文件后按 `Ctrl+S`，确认磁盘保存能力没有回归。
