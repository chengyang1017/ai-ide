...
export class CodeNoteController {
  ...
  constructor(
    ...
    this.popover.innerHTML = `
      ...
      <textarea class="code-note-textarea" placeholder="写下你的理解、TODO、问题……" spellcheck="false"></textarea>
  ...
  private openAtLine(line: number): void {
    const note = this.noteAtLine(line);
    this.draft = {
      id: note?.id ?? '',
      line,
      text: note?.text ?? '',
    ...
    this.lineLabel.textContent = `${this.filePath}:${line}`;
    this.deleteButton.hidden = !note;
  ...
  private async saveDraft(): Promise<void> {
    ...
    try {
      ...
      this.renderMarkers();
      this.renderAddMarker();
      this.onStatus(`📝 已保存便签 · ${saved.filePath}:${saved.line}`);
      this.close();
  ...
  private renderMarkers(): void {
    ...
    this.noteDecorations.set(this.notes.map((note) => ({
      range: new monaco.Range(note.line, 1, note.line, 1),
      options: {
        glyphMarginClassName: 'code-note-glyph',
        glyphMarginHoverMessage: { value: `📝 **代码便签**\n\n${this.preview(note.text)}` },
      },
    })));
  }

  private renderAddMarker(): void {
    if (!this.enabled) {
      this.addDecoration.clear();
      return;
    }
    ...
    if (!position || this.noteAtLine(position.lineNumber)) {
      this.addDecoration.clear();
      return;
    }
    ...
    this.addDecoration.set([{
      ...
      options: {
        ...
        glyphMarginHoverMessage: { value: '点击添加一个持久代码便签' },
      },
    }]);
  }

  private noteAtLine(line: number): CodeNote | undefined {
    return this.notes.find((note) => this.currentTrackedLine(note.id, note.line) === line);
  }
  ...
  private repositionPopover(): void {
    ...
    const line = this.currentTrackedLine(this.draft.id, this.draft.line);
    const runtimeModel = model as RuntimeTextModel;
    const lineEnd = this.editor.getScrolledVisiblePosition({
      lineNumber: line,
      column: runtimeModel.getLineMaxColumn(line),
    });
    ...
    const width = Math.min(340, Math.max(280, this.stage.clientWidth - 32));
    const height = 212;
    const preferredLeft = (lineEnd?.left ?? lineStart.left) + 18;
    const maxLeft = Math.max(8, this.stage.clientWidth - width - 12);
    const left = preferredLeft + width <= this.stage.clientWidth - 12
      ? preferredLeft
      : maxLeft;
    const top = Math.max(8, Math.min(lineStart.top + 24, this.stage.clientHeight - height - 10));
